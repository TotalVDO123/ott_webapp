...
export class PlansService {
  ...
  async getAvailablePlans({ siteId }: { siteId: string }): Promise<Plan[]> {
    try {
      ...
      logger.error('PlansService: getAvailablePlans: error fetching available plans:', e);
      throw e;
    }
  }
  ...
  async getEntitledPlans({ siteId, authorization }: { siteId: string; authorization?: string }): Promise<Plan[]> {
    try {
      ...
      logger.error('PlansService: getEntitledPlans: error fetching entitled plans:', e);
      throw e;
    }
  }
}
